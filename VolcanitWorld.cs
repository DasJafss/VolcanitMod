	...
	public class VolcanitWorld : ModWorld
	{
		...
		bool landed = false;
			for (ymodifier = 0; ymodifier < Main.maxTilesY - 1; ymodifier++) {
				...
				int[] SunkenTreasureExclus = new int[] {ItemType<Pickanch>(),ItemType<Riptide>(),ItemType<NautilusSlicer>()};
		...
		public override void PostWorldGen() {
			int toAdd = ModContent.ItemType<SwordOfTheOcean>();
			int toAdd2 = ModContent.ItemType<SwordOfTheJungle>();
			int toAdd3 = ModContent.ItemType<SwordOfTheTundra>();
			int toAdd4 = ModContent.ItemType<SwordOfTheDesert>();
			for (int chestIndex = 0; chestIndex < 1000; chestIndex++) {
				Chest chest = Main.chest[chestIndex];
